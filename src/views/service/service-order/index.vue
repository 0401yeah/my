<template>
  <div class="service-order-page flex flex-col gap-4 pb-5">
    <ArtSearchBar
      ref="searchBarRef"
...
const searchItems = computed(() => [
  ...
  },
  {
    ...
    options: [
      { label: '全部', value: '' },
      { label: '代泊服务', value: 'valet' },
      { label: '洗车服务', value: 'carwash' }
    ]
  },
  {
    key: 'status',
...
const fetchAllOrders = async (params: any) => {
  ...
  if (params.serviceType) {
    filteredOrders = filteredOrders.filter((order: any) => 
      order.serviceType === params.serviceType
    )
  }
...
const handleSearch = async () => {
  Object.assign(searchParams, {
    ...
    serviceType: searchFormState.value.serviceType,
...
const handleReset = () => {
  resetSearchParams()
  searchFormState.value = { orderNo: '', plateNumber: '', serviceType: '', status: '', createTime: [] }
  getData()
}
